(function () {
  ...
  function buildCalendar(root, data) {
    if (!root) return;
  ...
  document.querySelectorAll('script.calendar-data').forEach(function (dataScript) {
    var root = document.getElementById(dataScript.getAttribute('data-root'));
    var data = JSON.parse(dataScript.textContent);
    buildCalendar(root, data);
  });

  document.querySelectorAll('.cal-tabs').forEach(function (tabs) {
    var buttons = tabs.querySelectorAll('.cal-tab');
    buttons.forEach(function (btn) {
      btn.addEventListener('click', function () {
        buttons.forEach(function (b) { b.classList.remove('is-active'); });
        btn.classList.add('is-active');
        document.querySelectorAll('.cal-tab-pane').forEach(function (pane) {
          pane.classList.toggle('is-active', pane.id === btn.getAttribute('data-target'));
        });
      });
    });
  });
